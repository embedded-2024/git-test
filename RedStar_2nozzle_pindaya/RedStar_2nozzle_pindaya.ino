/*
  * Project Name: POS Dongle firmware for Hong yang (Red star) Fuel Dispenser
  * Description:  This code send data request to Fuel Dispenser and read data reply from Fuel Dispenser,  change these data to mqtt format and send to mqtt server as a client for one dispenser.
  * Author:       Myo Min Thant and Aung Kyaw Min Htet
  * Date:         25 Sep 2024
  * Version:      1.4
  * Hardware:     ESP32-WROOM-32E, Serial2
  * Used Library: Wifi.h, PubSubClient.h, WiFiClientSecure.h, EEPROM.h, ArduinoJson.h
  * Application:  Hong Yang (Red Start) Fuel Dispenser

  * Insalled :    Kyaw San Tatkon Device 3
*/
...
bool pump1_cancel = false;
bool pump2_cancel = false;
...
void EEPROMWrite() {
  ...
  EEPROM.write(109, 4);  //device id
  EEPROM.commit();
  EEPROM.write(110, 2);  // nozzle count
  EEPROM.commit();
  EEPROM.write(101, 7);  // nozzle id
  EEPROM.commit();
  EEPROM.write(102, 8);
  EEPROM.commit();
  EEPROM.write(103, 0);
    ...
    if (Buffer[bufcount - 2] == 0x12 && (Buffer[bufcount - 1] == 0x57 || Buffer[bufcount - 1] == 0x56 || Buffer[bufcount - 1] == 0x53 || Buffer[bufcount - 1] == 0x52)) {
      ...
      if (preset_check) {
        ...
        if (pumpapprocount1 == true && Buffer[0] == 0x01) {
          ...
          if (Buffer[0] == 0x01) pump1_cancel = true;
          else if (Buffer[0] == 0x02) pump2_cancel = true;
...
void pump_cancel() {
  ...
  if (Buffer[0] == 0x01 && pump1_cancel && pump1_status == "ide") {
    // Serial.println("This is pump one cancel");

    pumpidchange();

    client.publish(reqcancelbuf, mqttdatabuf);

    pump1_cancel = false;

  } else if (Buffer[0] == 0x02 && pump2_cancel && pump2_status == "ide") {
    // Serial.println("This is pump two cancel ");
...
void send_preset_fun() {
  ...
  if (preset_data_arr[0] == 0x01) pump1_status = "fuel";
  else if (preset_data_arr[0] == 0x02) pump2_status = "fuel";
...
void mqttfinal() {
  ...
  //     if (millis() / 1000 - previousMillis >= 5) {
