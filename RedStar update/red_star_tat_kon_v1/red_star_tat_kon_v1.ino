...
bool just_test = false;            //ak
String just_str;


void eepromtestwrite() {
  EEPROM.write(101, 3);
  EEPROM.commit();
  EEPROM.write(102, 4);
  EEPROM.commit();
...
void mqttfinal() {
  ...
  Serial.println("your total length is");
  Serial.println(total_state.length());

  Serial.println("your aryoucnt is");
  ...
  // for (int y = total_state.length() - 1; y >= 0; y--) {
  //   if (y == 6) {
  //     mqttdatabuf[arycount + y - 1] = '.';
  //     arycount--;
  //     Serial.println("ur is ");
  //     Serial.println(arycount + y - 1);
  //   }
  //   Serial.print(total_ary[y]);
  //   mqttdatabuf[arycount + y - 1] = total_ary[y];
  // }
