...
unsigned char read_state_3[4];
unsigned char read_state_4[4];
...
void EEPROMWrite() {
  EEPROM.write(109, 2);  //device id
  EEPROM.commit();
  EEPROM.write(110, 2);  // nozzle count
  EEPROM.commit();
  EEPROM.write(101, 3);  // nozzle id
  EEPROM.commit();
  EEPROM.write(102, 4);
  EEPROM.commit();
  EEPROM.write(103, 0);
  ...
  if (Serial2.available()) {
    ...
    if (Buffer[bufcount - 2] == 0x12 && (Buffer[bufcount - 1] == 0x57 || Buffer[bufcount - 1] == 0x56 || Buffer[bufcount - 1] == 0x53 || Buffer[bufcount - 1] == 0x52)) {
      ...
      if (preset_check) {
        ...
        } else if (pumpapprocount2 == true && Buffer[0] == 0x02) {
          send_app_fun();
          pumpapprocount2 = false;

        } else {
          // Serial.println("Approooooooooooooooooooooooooooooooooooooooooooooooooooooooo");
          for (int y = 0; y < 50; y++) {
            mqttdatabuf[y] = 0;
          }
    ...
    } else if (Buffer[bufcount - 2] == 0x12 && (Buffer[bufcount - 1] == 0x5F || Buffer[bufcount - 1] == 0x5B || Buffer[bufcount - 1] == 0x03)) {
      for (int i = 0; i < 8; i++) {
        Buffer[bufcount] = Serial2.read();
        delay(8);
        Serial.print(bufcount);
...
void send_preset_fun() {
  ...
  if (Buffer[0] == 0x02 && Buffer[2] == 0x89) {
...
void pricechangefun() {
  ...
  if (pumpid1 == pumpmqttnum3) {
    price1 = true;
    price_change_arr[0] = 0x01;

  } else if (pumpid2 == pumpmqttnum3) {
    price2 = true;
...
void presetfun() {  // pump preset
  ...
  byte OPP = calculate_checksum(preset_data_arr, 8);
