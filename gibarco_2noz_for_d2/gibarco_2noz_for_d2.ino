...
unsigned char pump1_stop[1] = { 0x31 };
unsigned char pump2_stop[1] = { 0x32 };
...
void main_fun() {
  ...
  if (Serial2.available() > 0) {
    ...
    if (Buffer[count - 1] == 0xF0 || Buffer[count - 1] == 0xD8 || Buffer[count - 1] == 0x31 || Buffer[count - 1] == 0x63) count = 0;

    else if (Buffer[count - 1] == 0xB1 && live1 == "ide" && live2 == "ide") count = 0;

    else if (Buffer[count - 1] == 0x81) {

      // digitalWrite(15, HIGH);
      // Serial2.write(pump1_stop, 1);
      // delay(2);
      // digitalWrite(15, LOW);
    ...
    // else if (Buffer[count - 1] == 0x61 || Buffer[count - 1] == 0x62) {
    ...
    //     if (nozzle1_count == 15) {
    //       Serial.println("nozzle 1 offfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
    //       digitalWrite(15, HIGH);
    //       Serial2.write(pump1_stop, 1);
    //       delay(2);
    //       digitalWrite(15, LOW);
    ...
    //   // if (nozzle1_count == 61) nozzle1_count = 0;
    ...
    // }
      ...
      if (Buffer[0] == 0x71 && pumpapprocount1) {

        preset_fun();

      } else if (Buffer[0] == 0x72 && pumpapprocount2) {
    ...
    } else if (Buffer[count - 1] == 0xB1 || Buffer[count - 1] == 0xA1 && live1 == "live") {
    ...
    } else if (Buffer[count - 1] == 0xB2 || Buffer[count - 1] == 0xA2 && live2 == "live") {
...
void read_fueled_data() {
  ...
  liter = String(tem_int2);
  price = String(tem_int3);
...
void mqttpplive() {
  ...
  Serial.print("price length is => ");
  Serial.println(mqtt_price.length());
  ...
  mqtt_liter.toCharArray(liter_ary, sizeof(liter_ary));
  mqtt_price.toCharArray(price_ary, sizeof(price_ary));

  for (int i = 0; i < mqtt_price.length(); i++) {
    Serial.print(price_ary[i], HEX);
    Serial.print(" ");
  }
...
void pumpidchange() {
  ...
  if (Buffer[0] == 0x91 || Buffer[0] == 0x71 || id_1) mqttpumpidchange(pumpid1);
  else if (Buffer[0] == 0x92 || Buffer[0] == 0x72 || id_2) mqttpumpidchange(pumpid2);
}
