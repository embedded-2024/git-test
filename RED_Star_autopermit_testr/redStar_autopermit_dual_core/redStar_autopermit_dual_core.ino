...
String master_card = "12345";
String accessToken;
int common_pump_Id;
bool appro_check = true;
  ...
  if (Serial2.available()) {
    ...
    if (Buffer[bufcount - 2] == 0x12 && (Buffer[bufcount - 1] == 0x57 || Buffer[bufcount - 1] == 0x56 || Buffer[bufcount - 1] == 0x53 || Buffer[bufcount - 1] == 0x52)) {
      ...
      if (preset_check) {
        ...
        //last  add
        if (lastpresetsend) {
          send_preset_fun();  /// test and add in akw station
          // presetfun();
...
void mqttfinal() {
  ...
  if (!reload_check_2 && !reload_check_1) {
    ...
    while (waitcount < 2) {
      ...
      if (millis() / 1000 - previousMillis >= 8) {
