...
unsigned char read_state_3[4] = { 0x01, 0x03, 0x12, 0xEA };
unsigned char read_state_4[4] = { 0x02, 0x03, 0x12, 0xE9 };
...
void EEPROMWrite() {
  ...
  EEPROM.commit();
  EEPROM.write(110, 1);  // nozzle count
  EEPROM.commit();
  ...
  if (Serial2.available()) {
    ...
    if (Buffer[bufcount - 2] == 0x12 && (Buffer[bufcount - 1] == 0x57 || Buffer[bufcount - 1] == 0x56 || Buffer[bufcount - 1] == 0x53 || Buffer[bufcount - 1] == 0x52)) {
      ...
      if (preset_check) {
        ...
        if (pumpapprocount1 && Buffer[0] == 0x01) {
          send_app_fun();
...
void send_read_state() {
  ...
  if (addresscount > nozzlenum) addresscount = 1;
...
void send_read_total() {
  ...
  if (addresscount > nozzlenum) addresscount = 1;  /// tatkone remove for one nozzle   address count is change 2 to 1
